...
Install steps:
\begin{itemize}
\item Download the \codeblocks installer (\url{https://codeblocks.org/downloads/26}). If you know you don't have MinGW installed, or if you don't know which one to choose, \textbf{download the package which has MinGW bundled}. For 17.12 version, the name of the installer is: codeblocks-17.12mingw-setup.exe. Next version will be something like 20.xx.
...
\begin{itemize}
\item The codeblocks-17.12-setup.exe file includes \codeblocks with all plugins. The codeblocks-17.12-setup-nonadmin.exe file is provided for convenience to users that do not have administrator rights on their machine(s).
\item The codeblocks-17.12mingw-setup.exe file includes additionally the GCC/G++ compiler and GDB debugger from TDM-GCC (version 5.1.0, 32 bit, SJLJ). The codeblocks-17.12mingw\_fortran-setup.exe file includes additionally to that the Gfortran compiler (TDM-GCC). \textbf{Unfortunately}, 5.1 version of gfortran is bugged (it's not TDM specific !). Next \codeblocks version will be distributed with an other MinGW version (so a new gfortran) which has not this problem.
\item The codeblocks-17.12(mingw)-nosetup.zip files are provided for convenience to users that are allergic against installers. However, it will not allow to select plugins / features to install (it includes everything) and not create any menu shortcuts. For the "installation" you are on your own.
\item Since 17.12 is quite old, there is the possibility to use a nightly build from the Forum. This builds does not come bundled with a compiler!! You need to install a compiler by yourself (if you have not already one). Before installing please have a look at \url{http://forums.codeblocks.org/index.php/topic,3232.0.html} 
\item A good solution, is to install an official distribution with MinGW and to install on the top of this official version a nightly. You will have to follow the procedure carefully because version 17.12 was compiled in 32 bits while nightly versions are now compiled in 64 bits. Mixing versions brings problems. 
\end{itemize}
...
If you install your nightly above a 17.12 version, you need to follow carefully this procedure because a few things have changed, at least they are compiled in 64 bits with a recent compiler, a different wxWidgets version and need some complementary dlls.
\begin{itemize}
