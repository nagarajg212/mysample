...
De retour dans le source principal (\textbf{main.cpp}) incluez le fichier d'entête et replacez la fonction \codeline{cout} afin de se conformer à la nouvelle configuration du projet.
...
Pressez Ctrl-F9, \menu{Générer,Générer}, ou \menu{Barre d'outil du Compilateur,Générer (bouton - la roue dentée)} pour compiler le projet. Si la sortie suivante s'affiche dans le journal de génération (dans le panneau du bas) c'est que toutes les étapes se sont correctement déroulées.
